...
export const fieldKeys = ["problemContext", "expectedResult", "acceptanceCriteria", "scope", "resources", "timingConstraints"] as const;
...
export const requestSchema = z.object({
  schemaVersion: z.literal(1), description: z.string().trim().min(1).max(8000),
...
export const outputSchema = z.object({
  schemaVersion: z.literal(1), missingFields: z.array(fieldSchema).max(6),
...
export type Analysis = z.infer<typeof outputSchema> & { mode: "openai" | "mock"; promptVersion: 1 };
...
export function fallback(input: AnalysisInput): Analysis {
  const order: Field[] = ["acceptanceCriteria", "scope", "resources", "timingConstraints", "problemContext", "expectedResult"];
  ...
  const selected = [...missing, ...order.filter(k => !missing.includes(k))].slice(0, Math.max(3, missing.length));
  return { schemaVersion: 1, missingFields: missing, questions: selected.map(field => ({
